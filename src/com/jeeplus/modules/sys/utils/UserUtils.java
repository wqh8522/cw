	...
	public static List<Role> getRoleList(){
		...
		if (roleList == null){
			...
			if (user.isAdmin()){
				...
				Role role = new Role();
				role.getSqlMap().put("dsf", BaseService.dataScopeFilter(user.getCurrentUser(), "o", "u", false));
	...
	public static List<Office> getOfficeList(){
		...
		if (officeList == null){
			...
			if (user.isAdmin()){
				...
				office.getSqlMap().put("dsf", BaseService.dataScopeFilter(user, "a", "", false));
...
//		getCacheMap().remove(key);
	 ...
	 * @return
	 */
	public static List<Office> getUserOtherOffice() {
		User user = getUser();
		String others = user.getOtherOrgIds();
		String[] ids = {};
		if(others!=null){
			 ids = others.split(",");
		}
		...
		for (String officeId : ids) {
			offices.add(officeDao.get(officeId));
		}
	 ...
	 * @return
	 */
	public static Integer getUserMaxDataScope() {
		List<Role> roles = getRoleList();
		Integer scope = 9;
		for (Role role : roles) {
			if (Integer.parseInt(role.getDataScope()) <= scope) {
				scope = Integer.parseInt(role.getDataScope());
			}
		}
