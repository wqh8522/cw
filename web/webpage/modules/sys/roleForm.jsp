		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      ...
		      <tr>
		         ...
		         <td  class="width-15" class="active">
		         ...
		         <td class="width-35">
	         		<form:select path="dataScope" class="form-control ">
	         			<c:if test="${userscope*1 <= 1}">
	         				<option value="1">${ fns:getDictLabel ("1", "sys_data_scope", "1")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 2}">
	         				<option value="1">${ fns:getDictLabel ("2", "sys_data_scope", "2")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 3}">
	         				<option value="1">${ fns:getDictLabel ("3", "sys_data_scope", "3")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 4}">
	         				<option value="4">${ fns:getDictLabel ("4", "sys_data_scope", "4")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 5}">
	         				<option value="5">${ fns:getDictLabel ("5", "sys_data_scope", "5")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 8}">
	         				<option value="8">${ fns:getDictLabel ("8", "sys_data_scope", "8")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 9}">
	         				<option value="9">${ fns:getDictLabel ("9", "sys_data_scope", "9")}</option>
	         			</c:if>
					</form:select>
		      ...
		      <tr>
		         <td class="width-15 active">
		         	<label class="pull-right">创建人:</label>
		         </td>
		         <td class="width-35">
		         	<sys:treeselect id="createBy" name="createBy" value="${role.createBy.id}" labelName="createBy.name" labelValue="${role.createBy.name}"
								title="用户" url="/sys/office/treeData?type=3" cssClass="form-control" notAllowSelectParent="true" />
				</td>
		         <td  class="width-15" class="active">
