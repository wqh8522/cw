...
<body class="hideScroll">
		<form:form id="inputForm" modelAttribute="news" action="${ctx}/dzjg/news/save" method="post" class="form-horizontal" enctype="multipart/form-data">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
				   ...
				   <td class="width-35" colspan="3">
					...
					<td class="width-35">
						<form:select path="type" class="form-control required">
							...
							<form:options items="${fns:getDictList('t_dzjg_news_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
					</td>--%>
			   ...
			   <tr>
				   ...
				   <td>
					   <c:if test="${not empty news.file}">
						   <a href="${ctx}/dzjg/dzjgFile/download?id=${news.file.id}">${news.file.fileName}</a>
					   </c:if>
					   <c:forEach items="${news.attachs}" varStatus="vs" var="att">
						   <a href="${ctx}/sys/attach/download?id=${att.id}" title="下载附件">
							   <c:if test="${news.attachs.size()>1}">
								   ${vs.count}、
							   </c:if>
								   ${att.fileName}
						   </a><br>
