 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/sys/log2")
public class LogController2 extends BaseController {

	@Autowired
	private LogService logService;
	
	@RequiresPermissions("sys:log2:list")
	@RequestMapping(value = {"list", ""})
	public String list(Log log, HttpServletRequest request, HttpServletResponse response, Model model) {
		log.setMethod("2");
		Page<Log> page = logService.findPage(new Page<Log>(request, response), log); 
        model.addAttribute("page", page);
		return "modules/sys/logList2";
	}
	 ...
	 * 批量删除
	 */
	@RequiresPermissions("sys:log2:del")
	...
	public String deleteAll(String ids, RedirectAttributes redirectAttributes, HttpServletRequest request) {
		...
		String loginfo = "删除登出日志";
		LogUtils.saveLog(request, loginfo);
		addMessage(redirectAttributes, "删除登出日志成功");
		return "redirect:"+Global.getAdminPath()+"/sys/log2/?repage";
	}
	
	/**
	 * 批量删除
	 */
	@RequiresPermissions("sys:log2:dell")
	@RequestMapping(value = "empty")
	public String empty(RedirectAttributes redirectAttributes, HttpServletRequest request) {
		logService.empty("2");
		String loginfo = "清空登出日志";
		LogUtils.saveLog(request, loginfo);
		addMessage(redirectAttributes, "清空登出日志成功");
		return "redirect:"+Global.getAdminPath()+"/sys/log2/?repage";
	}
}
