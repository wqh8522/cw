	...
	<script type="text/javascript">
		/*$(document).ready(function() {
            $("#planType").bind("change",function () {
                ...
                window.location = "${ctx}/dzjg/workplan/?planType="+planType;
            });
		});*/
	</script>
</head>
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="TDzjgWorkplan" action="${ctx}/dzjg/workplan/" method="post" class="form-inline">
		...
		<div class="form-group">
			...
			<span>单位：</span>
				<sys:treeselect id="office" name="office.id" value="${TDzjgWorkplan.office.id}" labelName="office.name" labelValue="${TDzjgWorkplan.office.name}"
			...
			<form:input path="startDate" id="startDate"
						value="${fns:formatDateTime1(TDzjgWorkplan.startDate,'yyyy-MM-dd')}"
			...
			<form:input path="stopDate" id="stopDate"
						value="${fns:formatDateTime1(TDzjgWorkplan.stopDate,'yyyy-MM-dd')}"
						onclick="laydate({ istime: true, format: 'YYYY-MM-DD'}); $('#laydate_box').css('-webkit-box-sizing', null);"
						class="form-control required  laydate-icon" />
		 </div>
	</form:form>
	...
	<div class="col-sm-12">
		<div class="pull-left">
			<%--<shiro:hasPermission name="dzjg:workplan:add">
				<table:addRow url="${ctx}/dzjg/workplan/form" title="计划管理"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>--%>
			<shiro:hasPermission name="dzjg:workplan:edit">
			    <table:editRow url="${ctx}/dzjg/workplan/form" title="工作计划" id="contentTable"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="dzjg:workplan:del">
				<table:delRow url="${ctx}/dzjg/workplan/deleteAll" id="contentTable"></table:delRow><!-- 删除按钮 -->
			</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<tr>
			...
			<%--<tr>
				...
				<th  class="sort-column planType">计划类型</th>
				<th  class="sort-column planTitle">计划标题</th>
				<th  class="sort-column planVontent">计划内容</th>
			...
			<tr>
				...
				<td>
					<shiro:hasPermission name="dzjg:workplan:view">
						<a href="#" onclick="openDialogView('查看工作计划', '${ctx}/dzjg/workplan/findPlanById?id=${workplan.id}','800px', '500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="dzjg:workplan:edit">
    					<a href="#" onclick="openDialog('修改工作计划', '${ctx}/dzjg/workplan/form?id=${workplan.id}','800px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="dzjg:workplan:del">
						<a href="${ctx}/dzjg/workplan/delete?id=${workplan.id}" onclick="return confirmx('确认要删除该工作计划吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
