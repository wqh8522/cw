...
public abstract class BaseService {
	 ...
	 * @param include 是否包含兼职机构数据
	 * @return 标准连接条件对象
	 */
	public static String dataScopeFilter(User user, String officeAlias, String userAlias, boolean include) {
				...
				for (String oa : StringUtils.split(officeAlias, ",")){
					if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)){
						if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())){
							...
							if (include) {
								// 兼职机构
								List<Office> offices = UserUtils.getUserOtherOffice();
