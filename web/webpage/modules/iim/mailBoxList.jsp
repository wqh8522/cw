                    ...
                    <table id="contentTable" class="table table-hover table-mail table-striped table-bordered">
                    	<thead> 
                    		<tr>
                                ...
                                <th class="sort-column mail_type">信息类型</th>
                        ...
                        <tbody>
                        	...
                        	<c:forEach items="${page.list}" var="mailBox" varStatus="vs">
								<tr>
									<td class="check-mail">
                                    ...
                                    <td class="mail-ontact"><a href="${ctx}/iim/mailBox/detail?id=${mailBox.id}">
                                        <c:if test="${mailBox.readstatus =='0'}">
                                            <b style="color:red">! </b><i class=" fa fa-envelope"> </i>
                                        </c:if>
                                        <c:if test="${mailBox.readstatus =='1'}">
                                            <i class="fa fa fa-envelope-o btn-white "></i>
                                        </c:if>
                                        <c:if test="${fn:length(mailBox.mail.title)>15 }">
                                            ${fn:substring(mailBox.mail.title, 0, 15)}...
                                        </c:if>
                                        <c:if test="${fn:length(mailBox.mail.title)<=15 }">
                                            ${mailBox.mail.title }
                                        </c:if>
                                    ...
                                    <td>
                                        ${fns:getDictLabel(mailBox.mail.mailType,"mail_type" ,"" )}
                                    </td>
	                                <td class=""><a href="${ctx}/iim/mailBox/detail?id=${mailBox.id}">
                                	
                                		 <c:if test="${fn:length(mailBox.sender.name)>5 }">  
                      				   ${fn:substring(mailBox.sender.name, 0, 5)}...  
                   					</c:if>  
                  					<c:if test="${fn:length(mailBox.sender.name)<=5 }">  
                        				 ${mailBox.sender.name }  
                  					</c:if>  
									...
									<td>
                                        <a href="${ctx}/iim/mailBox/detail?id=${mailBox.id}" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
