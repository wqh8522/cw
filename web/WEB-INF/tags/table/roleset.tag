...
<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="roleset()" title="设置角色"><i class="fa fa-users"></i> ${label==null?'设置角色':label}</button>
                        </button>
...
$(document).ready(function() {
	...
	function roleset(){
		  ...
		  if(size > 1 ){
		    ...
		    openDialog("设置角色","${url}?id="+id,"${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
		}
</script>